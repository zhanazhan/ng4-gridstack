export class GridStackOptions {
    ...
    animate: boolean;
    auto: boolean;
    cellHeight: string; // default is 60 could be an integer (px) a string (ex: '10em', '100px', '10rem') 0
                        // or null 'auto' - height will be calculated from cell width.
    ddPlugin: boolean;
    disableDrag: boolean;
    ...
    removable: boolean; // if true widgets could be removed by dragging outside of the grid. It could also be a jQuery
                        // selector string, in this case widgets will be removed by dropping them there (default: false)
    ...
    staticGrid: boolean; // makes grid static (default false). If true widgets are not movable/resizable. You don't even need
                         // jQueryUI draggable/resizable. A CSS class grid-stack-static is also added to the container.
